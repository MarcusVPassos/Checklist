            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 space-y-6">
                    ...
                    <form method="GET" action="{{ route('itens.index') }}" class="flex gap-2">
                        ...
                        @if (request('q'))
                            <a href="{{ route('itens.index') }}" class="px-3 py-2 rounded border">Limpar</a>
                        @endif
                    </form>
                        ...
                        <table class="min-w-full text-sm">
                            ...
                            <tbody class="divide-y divide-gray-200/60 dark:divide-gray-700/60">
                                @forelse($itens as $i)
                                    ...
                                    {{-- Modal Editar --}}
                                    @can('itens.update')
                                        <x-modal name="editar-item-{{ $i->id }}" :show="false" focusable
                                            maxWidth="2xl">
                                            <div class="p-6">
                    ...
                    {{-- Paginação --}}
                    <div>{{ $itens->withQueryString()->links() }}</div>
                </div>
            </div>
        </div>
    </div>
